...
if __name__ == "__main__":
    ...
    gfdl = [
       #lstmGraphFN(scope='lA_%s' % time.strftime('%m.%d_%H.%M'),                  optAda=False, lR=7e-4),
        cnnRGraphFN(scope='cB_%s' % time.strftime('%m.%d_%H.%M'), nLay=24,reW=512, optAda=False, lR=3e-4),
       #cnnRGraphFN(scope='cC_%s' % time.strftime('%m.%d_%H.%M'), nLay=12,reW=256,               lR=7e-7),
        cnnRGraphFN(scope='cD_%s' % time.strftime('%m.%d_%H.%M'), nLay=8, reW=768, optAda=False, lR=1e-4),
       #cnnRGraphFN(scope='cE_%s' % time.strftime('%m.%d_%H.%M'), nLay=8, reW=512,               lR=7e-7),
        cnnRGraphFN(scope='cF_%s' % time.strftime('%m.%d_%H.%M'), nLay=4, reW=1024,optAda=False, lR=7e-5),
       #cnnRGraphFN(scope='cG_%s' % time.strftime('%m.%d_%H.%M'), nLay=4, reW=512,               lR=7e-7),
    ]
